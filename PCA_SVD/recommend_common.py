...
def standEst(dataMat, user, simMeas, item):
    ...
    for j in range(n):
        ...
        if userRating == 0:  # 若此人未评价过该商品则不做下面处理
            continue
        overLap = nonzero(logical_and(dataMat[:, item].A > 0, dataMat[:, j].A > 0))[0]  # 获得相比较的两列同时都不为0的数据行号，找到对这两个item打分不为0的所有用户

        if len(overLap) == 0:
            similarity = 0
        else:
            # 求两列的相似度
            similarity = simMeas(dataMat[overLap, item], dataMat[overLap, j])  # 利用上面求得的两列同时不为0的行的列向量-》所有用户最这两个item的打分（打分不为0），以此来计算两个item的相似度
